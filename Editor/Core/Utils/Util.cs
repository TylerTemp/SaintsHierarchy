...
namespace SaintsHierarchy.Editor.Core.Utils
{
    public static class Util
    {
        ...
        public readonly struct PrefabIconInfo
        {
            public readonly bool IsInstanceRoot;
            public readonly bool IsMissingPrefab;
            public readonly bool IsModelPrefab;
            public readonly Texture2D Icon;

            public PrefabIconInfo(bool isInstanceRoot, bool isMissingPrefab, bool isModelPrefab, Texture2D icon)
            {
        ...
        public static PrefabIconInfo GetPrefabIconInfo(GameObject go)
        {
            if (go == null || !PrefabUtility.IsAnyPrefabInstanceRoot(go))
            {
                return default;
            }

            PrefabAssetType assetType = PrefabUtility.GetPrefabAssetType(go);
            bool isMissingPrefab = PrefabUtility.GetPrefabInstanceStatus(go) == PrefabInstanceStatus.MissingAsset;
            string iconName;
            if (isMissingPrefab)
            {
            ...
            {
                iconName = assetType switch
                {
                    PrefabAssetType.Regular => "d_Prefab Icon",
                    PrefabAssetType.Variant => "d_PrefabVariant Icon",
                    PrefabAssetType.Model => "d_PrefabModel Icon",
                    _ => null,
                };
            }

            Texture2D icon = iconName == null
                ? null
                : (Texture2D)EditorGUIUtility.IconContent(iconName).image;

            return new PrefabIconInfo(true, isMissingPrefab, assetType == PrefabAssetType.Model, icon);
        }
