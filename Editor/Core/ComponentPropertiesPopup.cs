...
namespace SaintsHierarchy.Editor.Core
{
    public class ComponentPropertiesPopup : EditorWindow
    {
        ...
        public static void Show(Component component, Rect activatorRect)
        {
            ...
            ShowAtScreenPosition(component, screenRect.position);
        }

        public static void Show(Component component, VisualElement activator)
        {
            if (component == null || activator.panel == null)
            {
                return;
            }

            foreach (EditorWindow owner in Resources.FindObjectsOfTypeAll<EditorWindow>())
            {
                if (owner.rootVisualElement.panel == activator.panel)
                {
                    ShowAtScreenPosition(component, owner.position.position + activator.worldBound.max);
                    return;
                }
            }
        }

        private static void ShowAtScreenPosition(Component component, Vector2 screenPosition)
        {
            if (component == null)
            {
                return;
            ...
            Rect initialPosition = new Rect(screenPosition.x, screenPosition.y, Width, FallbackHeight);
